...
void print_state(aliengohal::LowState state)
{
		// std::cout << "\33[H\33[2J" <<'\n'; //clear screen
		...
		// 
		std::cout << "*---------------------------------------------*" << '\n';
		std::cout << "MotorState state:" << '\n';
		std::cout << "LF : " << state.motorState[3].q << ", " << state.motorState[4].q	<< ", " << state.motorState[5].q << "  |	" << state.motorState[3].dq << ", " << state.motorState[4].dq  << ", " << state.motorState[5].dq << '\n';
		std::cout << "RF : " << state.motorState[0].q << ", " << state.motorState[1].q	<< ", " << state.motorState[2].q << "  |	" << state.motorState[0].dq << ", " << state.motorState[1].dq  << ", " << state.motorState[2].dq << '\n';
		std::cout << "LH : " << state.motorState[9].q << ", " << state.motorState[10].q  << ", " << state.motorState[11].q << "  |	" << state.motorState[9].dq << ", " << state.motorState[10].dq	<< ", " << state.motorState[11].dq << '\n';
		std::cout << "RH : " << state.motorState[6].q << ", " << state.motorState[7].q	<< ", " << state.motorState[8].q << "  |	" << state.motorState[6].dq << ", " << state.motorState[7].dq  << ", " << state.motorState[8].dq << '\n';
...
int main()
{
	std::cout << "Testing Aliengo-hal. Check the robot is suspended in the air!" << std::endl
						<< "Press Enter to continue..." << std::endl;
	std::cin.ignore();

	aliengohal::LowLevelInterface robot_interface;
	// std::shared_ptr<aliengohal::LowLevelInterface> robot_interface;
	// robot_interface = std::make_shared<aliengohal::LowLevelInterface>(aliengohal::LowLevelInterface());
	// unitree::InitEnvironment(); // memory lock -> Requires sudo

	aliengohal::LowState state = {0};
	aliengohal::LowCmd lowcmd = {0};
	...
	int motorIdxs[12] = {
		aliengohal::FL_0, aliengohal::FL_1, aliengohal::FL_2,
		aliengohal::FR_0, aliengohal::FR_1, aliengohal::FR_2,
		aliengohal::RL_0, aliengohal::RL_1, aliengohal::RL_2,
		aliengohal::RR_0, aliengohal::RR_1, aliengohal::RR_2,
	};
